...
export default async function (interaction: ButtonInteraction, vote: string) {
    ...
    const id = await verifyTypeAndFetchPollID(interaction.message.id, "cancel-observation");

    await db
        .insert(tables.cancelObservationVotes)
    ...
    await interaction.editReply(template.ok(await renderVote(id, interaction.user.id, "cancel-observation")));
