...
export default async function (interaction: ButtonInteraction) {
    ...
    if (affectedRows === 0) throw "This network user report is no longer pending.";
    ...
    await audit(interaction.user.id, "reports/reject", null, report.id, [interaction.message.id]);
